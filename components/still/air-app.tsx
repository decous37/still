...
import { PRISM_ENABLED } from './prism-enabled';
import type { LeaveGuard } from './prism/prism-app';
const PrismApp = lazy(() => import('./prism/prism-app'));
...
type View = 'practice' | 'settings' | 'complete' | 'prism';
...
export function AirApp() {
  const prismLeaveGuard = useRef<LeaveGuard>(null);
  ...
  useEffect(() => {
    const sync = () => {
      ...
      if (
        hash !== 'prism' &&
        prismLeaveGuard.current?.(`#${hash || 'practice'}`)
      ) {
        window.history.replaceState(null, '', '#prism');
        return;
      }
      if (hash === 'prism' && !PRISM_ENABLED)
        window.history.replaceState(null, '', '#practice');
      setCollectionOpen(hash === 'library');
      setView(
        hash === 'prism' && PRISM_ENABLED
          ? 'prism'
          : hash === 'settings' || hash === 'complete'
            ? hash
            : 'practice',
      );
    };
  ...
  useEffect(() => {
    document.title = `Still — ${view === 'prism' ? (lang === 'en' ? 'Prism' : '幻彩') : c[view === 'complete' ? 'practice' : view]}`;
  }, [c, view]);
  useEffect(() => {
    try {
  ...
  return (
    <>
      <div className="air-app" hidden={view === 'prism'}>
        <a className="skip-link" href="#main">
          {lang === 'en' ? 'Skip to content' : '跳到内容'}
        </a>
        <header className="air-header">
          <div className="still-brand-group">
            <a
              className="air-brand"
              href="#practice"
              onClick={() => {
            ...
            {PRISM_ENABLED && (
              <a className="prism-entry" href="#prism">
                {lang === 'en' ? 'Prism' : '幻彩'}
              </a>
            )}
          </div>
      ...
      {view === 'prism' && PRISM_ENABLED && (
        <Suspense
          fallback={
            <div className="still-loading">
              {lang === 'en' ? 'Loading Prism…' : '正在打开幻彩…'}
            </div>
          }
        >
          <PrismApp
